//! Plan / sign / encode a local proposal on an accepted duty tick.

use crate::block_builder::{
    decide_publish, encode_proposal_gossip, plan_from_pool, try_attach_type2_from_type1_cache,
...
/// Plan a proposal from the Type-1 cache, attach Type-2 when allowed, and queue gossip.
pub fn try_plan_proposal(owner: &mut ChainOwner, tick: DutyTick) -> Vec<ChainEvent> {
    ...
    if publish_allowed {
        // D3: mesh proposals require Type-1 cache hits for every packed attestation.
        let require_cache = !owner.local_finality;
        if let Type2CacheAttach::Attached {
            proof_len,
            type1_hits,
            type1_needed,
        } = try_attach_type2_from_type1_cache(&mut plan, &owner.aggregates, require_cache)
        {
            let root = plan.block_root().unwrap_or([0u8; 32]);
            out.push(ChainEvent::Type2ProofAttached {
                root,
                proof_len,
                type1_hits,
                type1_needed,
