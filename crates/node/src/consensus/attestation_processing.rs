    ...
    fn verify_attestation_signature(
        ...
        for (i, &validator_index) in committee.validators.iter().enumerate() {
            if i < attestation.aggregation_bits.len() && attestation.aggregation_bits[i] {
                ...
                if let Some(validator) = state.validators.get(validator_index.as_usize()) {
        ...
        for (i, &participated) in attestation.aggregation_bits.iter().enumerate() {
            if participated {
                ...
                if let Some(&validator_index) = committee.validators.get(i) {
                    if let Some(validator) = state.validators.validators.get(validator_index.as_usize()) {
    ...
    fn create_test_attestation(slot: Slot, committee_index: u64) -> Attestation {
        Attestation {
            ...
            data: AttestationData {
                ...
                source: crate::types::Checkpoint { epoch: Epoch::ZERO, root: [0u8; 32] },
                target: crate::types::Checkpoint { epoch: Epoch::new(1), root: [1u8; 32] },
            },
