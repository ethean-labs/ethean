...
impl Default for StateTransitionConfig {
    fn default() -> Self {
        let profile = ethean_profile::lstar_devnet().expect("lstar profile must validate");
        Self {
            // Not in lstar ChainProfile yet; callers must keep this explicit.
            slots_per_epoch: 32,
...
impl StateTransitionProcessor {
    ...
    fn get_block_proposer(&self, state: &BeaconState, slot: Slot) -> Result<ValidatorIndex, StateTransitionError> {
        ...
        Ok(ValidatorIndex::new(proposer_index))
    }
    ...
    fn process_validator_registry_updates(
        ...
        for (index, validator) in state.validators.iter_mut().enumerate() {
            ...
            if validator.activation_eligibility_epoch == u64::MAX 
                ...
                activation_queue.push((ValidatorIndex::new(index as u64), epoch + 1));
            }
