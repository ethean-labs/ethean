...
pub fn enable_arena() {
    ...
    // Upstream leanVM arenas rely on sparse Unix mmap. On Windows keep the System
    // allocator so we never reserve multi-GiB slabs via VirtualAlloc.
    #[cfg(not(windows))]
    {
    ...
    #[cfg(windows)]
    {
        ARENA_ENGAGED.store(false, Ordering::Release);
    }
}
