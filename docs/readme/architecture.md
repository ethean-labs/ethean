...
Ethean is a **Lean Consensus** client workspace, not a Beacon Chain node and not
an execution client.
...
`ChainOwner` is the sole writer of head/sync flags. Workers read snapshots and
send `ChainCommand`s.

## Open gates

QUIC transport, RocksDB backend, leanSig/leanVM FFI, and full duty-loop wiring
continue to harden against pq-devnet operator pins.

Details:
[road-to/lean-consensus-migration/README.md](../../road-to/lean-consensus-migration/README.md).
